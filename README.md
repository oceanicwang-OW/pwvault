...
当前开发已推进到 **M2 桌面 MVP 的 T2.9：详情面板与密码行状态机**。下一项顺序任务是 **T2.10：条目编辑表单与内嵌密码生成器**；T2.9 目前按约定只在 mock 数据上实现详情交互（明文解密为 mock），真实条目数据与 CRUD 后续再接入。
...
| T2.9 详情面板与密码行状态机 | 已完成（mock） | 当前分支 | 通用 FieldRow、密码行状态机（8 点掩码↔明文 10s 倒计时回隐、切后台/锁定立即回隐、引用置空）、复制反馈倒计时条；明文解密为 mock。 |
...
1. 实现条目编辑表单与内嵌密码生成器：标题必填校验、标签多选、URL 规范化、生成器弹层与强度条、保存后列表即时刷新。
