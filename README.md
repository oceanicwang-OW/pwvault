...
当前开发已推进到 **M2 桌面 MVP 的 T2.10：条目编辑表单与内嵌密码生成器**。下一项顺序任务是 **T2.11：全局快捷键**；T2.10 目前按约定把新建/编辑结果写入共享 mock 存储（`mockEntryStoreProvider`）以演示"保存→列表即时刷新"，真实库 CRUD 后续替换。
...
| T2.10 条目编辑与密码生成器 | 已完成（mock） | 当前分支 | 新建/编辑表单（标题必填、标签多选、URL 规范化、脏数据二次确认）、内嵌生成器（长度/字符集/短语+强度条）、共享 mock 存储保存后列表即时刷新。 |
...
1. 实现全局快捷键：Ctrl/Cmd+F 聚焦搜索、Ctrl/Cmd+L 锁定、Ctrl/Cmd+N 新建、Enter 复制选中条目密码、↑↓ 列表导航。
2. 实现真实解锁流程：库文件选择、创建库、输入主密码、错误提示和递增等待。
3. 实现条目新增、编辑、删除、恢复和密码显示/复制交互。
4. 接入设置页：自动锁定时长、剪贴板清理时长、主题、修改主密码。
5. 设计并实现同步/合并流程。
6. 补齐 TOTP、导入导出和发布打包。
