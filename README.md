...
M2 桌面 UI 任务（T2.6–T2.12）已全部落地，当前转入**业务接通**：已完成 **T2.6 解锁页真实化（核心）**——解锁页接通真实 `VaultService`，按库文件是否存在自动进入解锁/建库模式（建库走 zxcvbn 强度门槛），错误抖动 + 连续 5 次后禁用输入并显示 2ⁿ 秒递增倒计时，成功后进入主界面。库文件固定在应用文档目录 `pwvault/vault.pwvault`（path_provider）；file_picker 多库选择与最近库列表持久化留作后续任务。下一步：把列表/详情/编辑的 mock 数据替换为真实库 CRUD。
...
| T2.6 解锁页 | 已完成（核心真实化） | `2905611`, `<pending>` | 接通真实 `VaultService`：库存在→解锁、不存在→建库（主密码+确认+zxcvbn 强度门槛）；错误抖动 + 连续 5 次后禁用输入并显示 2ⁿ 秒递增倒计时；库固定于应用文档目录（path_provider），成功后进主界面；9 项 widget test。file_picker 多库 + 最近库列表持久化留待后续。 |
...
1. 把列表/详情/编辑的 mock 数据替换为真实库 CRUD，并接通选中条目→详情。
2. 解锁页补全：file_picker 选择其他库文件、最近打开库列表持久化（库选择器下拉）。
