...
| T2.8 列表栏搜索与结果交互 | 已完成（真实数据） | `421f297`, `<pending>` | 搜索框 150ms debounce、Esc 清空、命中高亮、拼音匹配标注、空态引导新建；数据源接真实 `entryListProvider`，索引按条目列表缓存。 |
| T2.9 详情面板与密码行状态机 | 已完成（真实数据） | `40271e3`, `<pending>` | 通用 FieldRow、密码行状态机（8 点掩码↔明文 10s 倒计时回隐、切后台/锁定立即回隐、引用置空）、复制反馈倒计时条；选中条目驱动详情，明文走真实 `revealPassword` 单条目按需解密，编辑/改常用写真实库；未选中显示空态。 |
| T2.10 条目编辑与密码生成器 | 已完成（真实数据） | `eb94323`, `<pending>` | 新建/编辑表单（标题必填、标签多选、URL 规范化、脏数据二次确认）、内嵌生成器（长度/字符集/短语+强度条）；保存写真实 `VaultService.upsert`（异步 + 提交态 + 错误提示），列表即时刷新。 |
...
1. 解锁页补全：file_picker 选择其他库文件、最近打开库列表持久化（库选择器下拉）。
2. 侧边栏「常用/回收站/标签」过滤接真实数据（含回收站恢复/清空）；条目删除入口。
3. 把设置页的主题/时长偏好接入真实持久化（落盘，与库外本地配置协调）。
4. 设计并实现同步/合并流程。
5. 补齐 TOTP、导入导出和发布打包。
