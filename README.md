...
当前开发已推进到 **M2 桌面 MVP 的 T2.8：列表栏搜索与结果交互**。下一项顺序任务是 **T2.9：详情面板与密码行状态机**；T2.8 目前按约定只在 mock 数据上实现搜索交互，真实条目数据与 CRUD 后续再接入。
...
| T2.8 列表栏搜索与结果交互 | 已完成（mock） | 当前分支 | 搜索框 150ms debounce、Esc 清空、命中高亮、拼音匹配标注、空态引导新建；数据仍为 mock。 |
...
1. 实现详情面板与密码行状态机：FieldRow 通用组件、掩码↔明文 10s 倒计时回隐、复制反馈与倒计时条。
