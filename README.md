...
当前开发已推进到 **M2 桌面 MVP 的 T2.11：全局快捷键**。下一项顺序任务是 **T2.12：设置页**；T2.11 把列表搜索词/选中项提到共享 provider 以支撑快捷键，数据与明文复制仍为 mock，真实库 CRUD 后续替换。
...
| T2.11 全局快捷键 | 已完成（mock） | 当前分支 | Ctrl/Cmd+F 搜索、Ctrl/Cmd+L 锁定、Ctrl/Cmd+N 新建、Enter 复制选中密码、↑↓ 导航；`core/shortcuts.dart` + 列表状态 provider 化，五组快捷键 widget test 全过。 |
...
1. 实现设置页：自动锁定时长、剪贴板清除时长、主题切换、修改主密码（旧密码验证→新密码强度门槛）。
