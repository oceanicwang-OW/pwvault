...
当前开发已推进到 **M2 桌面 MVP 的 T2.7：桌面三栏主界面 Shell**。下一项顺序任务是 **T2.8：列表栏搜索与结果交互**；T2.7 目前按约定只实现 Shell 和 mock 数据，真实条目数据与 CRUD 后续再接入。
...
| T2.7 桌面三栏主界面 | 已完成（Shell） | 当前分支 | 侧边栏 140px、列表栏 220px、详情区自适应；窄屏折叠侧边栏为汉堡抽屉。 |
...
1. 实现列表栏搜索与结果交互：150ms debounce、Esc 清空、命中高亮、空态。
