...
| T2.14 库选择器 + 最近库持久化 | 已完成 | `<pending>` | 解锁页库选择器下拉：最近打开库列表 + 「打开其他库文件…」（file_picker）；选中库切换解锁/建库模式，成功后记录到库外 `config.json`（去重/置顶/限 8，明文不参与同步，存储不可用时降级）；新增 `services/local_config.dart`；4 项 widget/单元 test。 |
...
1. 把设置页的主题/时长偏好接入真实持久化（复用 `local_config.dart` 落盘）。
2. 回收站永久清空（待 Rust 侧 hard-delete，T5.4）。
3. 同步/合并流程；TOTP、导入导出、发布打包。
4. 设计并实现同步/合并流程。
5. 补齐 TOTP、导入导出和发布打包。
