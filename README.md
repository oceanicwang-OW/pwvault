...
| T2.13 侧边栏过滤 + 回收站 + 删除 | 已完成 | `<pending>` | 侧边栏「全部/常用/标签」客户端过滤 + 实时计数与标签聚合；回收站接 `listTrash`/`restore`，详情「删除（移到回收站）」与「恢复」；过滤切换清空选中；3 项过滤/回收站 widget test。永久清空回收站待 Rust hard-delete（T5.4）。 |
...
2. 把设置页的主题/时长偏好接入真实持久化（落盘，与库外本地配置协调）。
3. 回收站永久清空（待 Rust 侧 hard-delete，T5.4）。
4. 设计并实现同步/合并流程。
5. 补齐 TOTP、导入导出和发布打包。
